...
3. Aktif request ve tool cancellation fonksiyonları çağrılır.
4. Mock server en fazla üç saniye içinde durdurulur.
5. Başlamış IPC çağrılarının tamamlanması beklenir.
...
1. `bridgeMethodNames` browser tarafına sunulacak isimlerin allowlist’idir.
...
çağrılabilmesi için yeterli değildir.
...
Paylaşılan state `Bridge.mu` ile korunur. Cancellation kayıtları silinirken
...
Tek global store `useWorkspaceStore`’dur. Sorumlulukları:
...
Zustand `persist` middleware’i `localStorage` kullanır:
...
IPC runtime her dispatch için goroutine başlatır. Şu anda global/per-method
concurrency limiti veya toplam response payload backpressure mekanizması yoktur.
Yeni pahalı bridge metodunda aşağıdakiler tasarlanmadan sınırsız fan-out
oluşturulmamalıdır:
...
- Secret header doğrudan değeri persist edilmez.
...
2. IPC’ye global/per-method concurrency ve aggregate output backpressure
   eklenmeli.
3. Persistence, URL/body secret riskini daha açık ele almalı; localStorage
   access hataları güvenli adapter arkasına alınmalı.
4. Request execution `running: boolean` yerine discriminated union olmalı.
5. `UserError.code` ve runner failure code named enum/constant kümelerine
   taşınmalı.
...
3. Go/TypeScript IPC sözleşmesi schema veya code generation ile
   senkronize edilmeli.
4. Mock sample generator `writeOnly`, `uniqueItems`, `multipleOf` ve `pattern`
   gibi constraint’lerde genişletilmeli.
5. Desktop request body policy’si runner ile karşılaştırılıp tek açık
   semantiğe bağlanmalı.
6. Büyük tool’ların mounted tutulması için bellek ölçümü ve eviction politikası
   değerlendirilmeli.
7. Tek büyük `styles.css` feature/token katmanlarına ayrılırken cascade ve tema
...
| IPC metodu | `internal/canbridge/invoke.go`, `frontend/src/lib/backend.ts` |
...
| Collection | `internal/runner`, `internal/assertions`, `features/automation` |
