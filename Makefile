...
.PHONY: frontend-deps dev build build-cli install-linux test
...
dev: frontend-deps
	...
	cd $(FRONTEND_DIR) && ./node_modules/.bin/vite --host "$(DEV_HOST)" --port "$$dev_port" --strictPort & \
	vite_pid=$$!; \
	windows_resource=""; \
	trap 'kill "$$vite_pid" 2>/dev/null || true; if [ -n "$$windows_resource" ]; then rm -f "$$windows_resource"; fi' EXIT INT TERM; \
	attempt=0; \
